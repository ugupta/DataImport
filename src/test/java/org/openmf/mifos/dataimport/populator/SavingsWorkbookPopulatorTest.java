...
public class SavingsWorkbookPopulatorTest {
    ...
    public void shouldPopulateSavingsWorkbook() {
		...
		SavingsWorkbookPopulator savingsWorkbookPopulator = new SavingsWorkbookPopulator(new ClientSheetPopulator(restClient),
				new PersonnelSheetPopulator(onlyLoanOfficers, restClient), new SavingsProductSheetPopulator(restClient));
    	...
    	Assert.assertEquals("Field Officer*", row.getCell(3).getStringCellValue());
    	...
    	Assert.assertEquals("Currency", row.getCell(7).getStringCellValue());
    	Assert.assertEquals("Decimal Places", row.getCell(8).getStringCellValue());
    	Assert.assertEquals("In Multiples Of", row.getCell(9).getStringCellValue());
    	Assert.assertEquals("Interest Rate %*", row.getCell(10).getStringCellValue());
    	Assert.assertEquals("Interest Compounding Period*", row.getCell(11).getStringCellValue());
    	Assert.assertEquals("Interest Posting Period*", row.getCell(12).getStringCellValue());
    	Assert.assertEquals("Interest Calculated*", row.getCell(13).getStringCellValue());
    	Assert.assertEquals("# Days in Year*", row.getCell(14).getStringCellValue());
    	Assert.assertEquals("Min Opening Balance", row.getCell(15).getStringCellValue());
    	Assert.assertEquals("Locked In For", row.getCell(16).getStringCellValue());
    	Assert.assertEquals("Withdrawal Fee", row.getCell(18).getStringCellValue());
    	Assert.assertEquals("Annual Fee", row.getCell(20).getStringCellValue());
    	Assert.assertEquals("On Date", row.getCell(21).getStringCellValue());
    	Assert.assertEquals("Apply Withdrawal Fee For Transfers", row.getCell(22).getStringCellValue());
